...
最新の統合検証では `swift build` が成功し、27スイート・137テストが全件成功しています。
...
- サイドバーはFinder相当の高密度な18pt行。フォルダサイズ計算中はファイル名を出さず件数とスピナーのみ表示
...
- サイドバー幅変更はドラッグ開始時の幅を固定し、終了時だけ保存するためレイアウトのフィードバック振動を抑止
