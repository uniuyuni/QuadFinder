...
最新の統合検証では `swift build` が成功し、35スイート・223テストが全件成功しています。
...
- ペインごとに独立した複数タブ。各タブがURL、履歴、選択、表示設定、bookmarkを保持。タブ操作メニューの開閉インジケーターはmacOS標準の1個だけを表示
...
- 行全体での選択、余白を抑えた20ptのFinder風サイドバー、18ptの高密度ファイルリスト、アイコン表示、任意深度カラム／ツリー表示。ツリーのフォルダ矢印は閉鎖時右向き・展開時下向きに同期
- ペインごとのクリック可能なパンくずパスと、項目数・選択数・選択容量を示すステータス行
- Finder風サイドバー（よく使う項目・カスタムbookmark・Trash・マウント済みデバイス）。「よく使う項目」は単一のネイティブNSTableViewが選択とD&Dを所有し、行中央へのD&DはFinder準拠のコピー／移動／リンク、Trash行はゴミ箱移動、標準の挿入線へのフォルダD&Dだけがお気に入り追加となる。専用private pasteboardによって既定項目を含めて安定して並べ替えでき、順序・表示幅・表示状態を保存。全セクションは20pt行・12pt文字・16ptアイコンへ統一し、既定項目はaccent色のSF Symbol、追加フォルダはNSWorkspaceのカラーアイコンを使用。ネイティブ表は上端・左端・全幅へ固定され、項目数×20ptの決定論的な高さでList全体のスクロールに参加
...
- マウスイベントを消費しないペイン入力ルーター。1クリック選択、Command複数／Shift範囲選択、選択行からのドラッグを共存。ポインタから開く経路はネイティブ表示が明示したcontent URLだけを受理し、行末余白・別カラム・ツリー開閉矢印・空き領域・ペイン全域イベント監視から現在選択を開くフォールバックはない
...
- サイドバーはFinder相当の高密度な20pt行（上下の過大余白なし）。フォルダサイズ計算中はファイル名を出さず件数とスピナーのみ表示
...
- 軽量テキストエディタモジュール（AppKitネイティブ編集、行番号、検索、行移動、折返し、等幅フォント、tab幅、不可視文字、dirty表示、Cmd-S、任意2秒autosave）
- UTF-8 BOM有無、UTF-16/32 LE/BE、Shift_JIS、EUC-JP、ISO-2022-JPとLF/CRLF/CRの判定・維持。バイナリはHexビューアーへ誘導
- 同一ディレクトリ一時ファイル、同期、外部stamp再検証、mode/xattr維持、atomic renameによる安全保存。外部変更時は再読込／上書き／別名／比較を選択
- 100MBまたは100万行超は64KiBページ・最大64MB LRU・2MiB仮想ウィンドウへ切替。前後移動、byte/line移動、全体検索、進捗、中止に対応
- 1GB未満の大容量UTF-8は明示切替後にfile-backed Piece Tableで編集し、全体を展開せずstream保存。1GB以上は安全のため読み取り専用
- 通常／大容量テキスト保存をファイル操作履歴へ記録し、保存前後backupとfingerprintを検証してUndo/Redo
- テキスト本文にフォーカスがあるCommand+Sは現在の文書を保存し、Command+Aは本文（検索・行入力欄ではその入力欄）を全選択。ペイン側のCommand+Aはアクティブペインで表示中の全項目を選択し、コピー／カット等もfirst responderを優先
- 未保存の通常／大容量テキストを開いたまま別ファイル、フォルダ、複数項目または未選択へ切り替える場合は「保存／変更を破棄／キャンセル」を表示。保存失敗とキャンセルでは元文書・元ペイン選択を維持し、連続した選択変更は最新候補へ集約
- 未保存のテキストエディタを×またはモジュールメニューから閉じる場合も同じ保存確認を使用。キャンセル・保存失敗ではエディタを閉じず、文書・選択・入力フォーカスを維持し、閉じる際は古いAppKit first responderを解放
